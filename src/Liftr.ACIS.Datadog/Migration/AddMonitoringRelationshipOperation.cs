...
namespace Microsoft.Liftr.ACIS
{
    ...
    /// </summary>
    public class AddMonitoringRelationshipOperation : AcisSMEOperation
    {
        public override string OperationName => "AddMonitoringRelationship";
        ...
        /// </summary>
        public IAcisSMEOperationResponse AddMonitoringRelationship(string resourceId, string diagnosticSettingsName, string monitorId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            return AddMonitoringRelationshipAsync(resourceId, diagnosticSettingsName, monitorId, extension, updater, endpoint).Result;
        }

        private async Task<IAcisSMEOperationResponse> AddMonitoringRelationshipAsync(
            ...
            IAcisSMEEndpoint endpoint = null)
        {
            ...
            var result = await coordinator.StartWorkAsync(nameof(AddMonitoringRelationship), parameters: ConcatParams(monitoredResourceId, diagnosticSettingsName, monitorId));
            if (result.Succeeded)
            {
