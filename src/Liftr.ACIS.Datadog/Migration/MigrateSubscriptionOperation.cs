...
{
    public class MigrateSubscriptionOperation : AcisSMEOperation
    {
        public override string OperationName => "MigrateSubscription";
        ...
        /// </summary>
        public IAcisSMEOperationResponse MigrateSubscription(string monitorId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            return MigrateSubscriptionAsync(monitorId, extension, updater, endpoint).Result;
        }

        private async Task<IAcisSMEOperationResponse> MigrateSubscriptionAsync(string monitorId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            if (extension == null)
            {
            ...
            var result = await coordinator.StartWorkAsync(nameof(MigrateSubscription), parameters: monitorId);
            if (result.Succeeded)
            {
