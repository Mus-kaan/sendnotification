...
{
    public class MigrateMonitorOperation : AcisSMEOperation
    {
        public override string OperationName => "MigrateMonitor";
        ...
        /// </summary>
        public IAcisSMEOperationResponse MigrateMonitor(string monitorId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            return MigrateMonitorAsync(monitorId, extension, updater, endpoint).Result;
        }

        private async Task<IAcisSMEOperationResponse> MigrateMonitorAsync(string monitorId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            if (extension == null)
            {
            ...
            var result = await coordinator.StartWorkAsync(nameof(MigrateMonitor), parameters: monitorId);
            if (result.Succeeded)
            {
