...
namespace Microsoft.Liftr.ACIS
{
    ...
    /// </summary>
    public class SendNotificationsOperation : AcisSMEOperation
    {
        public override string OperationName => "SendNotifications";
        ...
        public override IEnumerable<IAcisSMEParameterRef> Parameters => new[]
            {
                ParamRefFromParam.Get<DescriptionIdTextParameter>(isOptional: false),
            };
        ...
        /// </summary>
        public IAcisSMEOperationResponse SendNotifications(string monitorId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            return SendNotificationsAsync(monitorId, extension, updater, endpoint).Result;
        }

        private async Task<IAcisSMEOperationResponse> SendNotificationsAsync(string monitorId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            if (extension == null)
            {
            ...
            var result = await coordinator.StartWorkAsync(nameof(SendNotifications), parameters: monitorId);
            if (result.Succeeded)
            {
