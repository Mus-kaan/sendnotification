...
namespace Microsoft.Liftr.ACIS
{
    ...
    /// </summary>
    public class GetMarketplaceResourceByMonitorOperation : AcisSMEOperation
    {
        public override string OperationName => "GetMarketplaceResourceByMonitor";
        ...
        /// </summary>
        public IAcisSMEOperationResponse GetMarketplaceResourceByMonitor(string monitorId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            return GetMarketplaceResourceByMonitorAsync(monitorId, extension, updater, endpoint).Result;
        }

        private async Task<IAcisSMEOperationResponse> GetMarketplaceResourceByMonitorAsync(string monitorId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            if (extension == null)
            {
            ...
            var result = await coordinator.StartWorkAsync(nameof(GetMarketplaceResourceByMonitor), parameters: monitorId);
            if (result.Succeeded)
            {
