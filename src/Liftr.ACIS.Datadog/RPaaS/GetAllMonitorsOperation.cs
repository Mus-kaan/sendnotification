...
namespace Microsoft.Liftr.ACIS
{
    ...
    /// </summary>
    public class GetAllMonitorsOperation : AcisSMEOperation
    {
        public override string OperationName => "GetAllMonitors";
        ...
        /// </summary>
        public IAcisSMEOperationResponse GetAllMonitors(string monitorResourceId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            return GetAllMonitorsAsync(monitorResourceId, extension, updater, endpoint).Result;
        }

        private async Task<IAcisSMEOperationResponse> GetAllMonitorsAsync(string monitorResourceId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            if (extension == null)
            {
            ...
            var result = await coordinator.StartWorkAsync(nameof(GetAllMonitors), parameters: monitorResourceId);
            if (result.Succeeded)
            {
