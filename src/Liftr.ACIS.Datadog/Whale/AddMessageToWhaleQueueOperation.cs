...
namespace Microsoft.Liftr.ACIS
{
    ...
    /// </summary>
    public class AddMessageToWhaleQueueOperation : AcisSMEOperation
    {
        public override string OperationName => "AddMessageToWhaleQueue";
        ...
        /// </summary>
        public IAcisSMEOperationResponse AddMessageToWhaleQueue(string monitorId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            return AddMessageToWhaleQueueAsync(monitorId, extension, updater, endpoint).Result;
        }

        private async Task<IAcisSMEOperationResponse> AddMessageToWhaleQueueAsync(string monitorId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            if (extension == null)
            {
            ...
            var result = await coordinator.StartWorkAsync(nameof(AddMessageToWhaleQueue), parameters: monitorId);
            if (result.Succeeded)
            {
