...
namespace Microsoft.Liftr.ACIS
{
    ...
    /// </summary>
    public class GetStatusByMonitorOperation : AcisSMEOperation
    {
        public override string OperationName => "GetStatusByMonitor";
        ...
        /// </summary>
        public IAcisSMEOperationResponse GetStatusByMonitor(string monitorId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            return GetStatusByMonitorAsync(monitorId, extension, updater, endpoint).Result;
        }

        private async Task<IAcisSMEOperationResponse> GetStatusByMonitorAsync(string monitorId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            if (extension == null)
            {
            ...
            var result = await coordinator.StartWorkAsync(nameof(GetStatusByMonitor), parameters: monitorId);
            if (result.Succeeded)
            {
