...
namespace Microsoft.Liftr.ACIS
{
    ...
    /// </summary>
    public class GetExistingRelationshipsOperation : AcisSMEOperation
    {
        public override string OperationName => "GetExistingRelationships";
        ...
        /// </summary>
        public IAcisSMEOperationResponse GetExistingRelationships(string monitorId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            return GetExistingRelationshipsAsync(monitorId, extension, updater, endpoint).Result;
        }

        private async Task<IAcisSMEOperationResponse> GetExistingRelationshipsAsync(string monitorId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            if (extension == null)
            {
            ...
            var result = await coordinator.StartWorkAsync(nameof(GetExistingRelationships), parameters: monitorId);
            if (result.Succeeded)
            {
