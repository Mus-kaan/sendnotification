...
namespace Microsoft.Liftr.ACIS.Dynatrace
{
    ...
    /// </summary>
    public class GetMarketplaceResourceIdByMonitorOperation : AcisSMEOperation
    {
        /// <summary>
        /// Name of the operation. This is prominently visible from Jarvis. One search an operation by name.
        /// </summary>
        public override string OperationName => "GetMarketplaceResourceIdByMonitor";
        ...
        /// for example this class name is GetMarketplaceResourceIdByMonitorOperation and thus method name is GetMarketplaceResourceIdByMonitor()
        /// </summary>
        ...
        /// <returns></returns>
        public IAcisSMEOperationResponse GetMarketplaceResourceIdByMonitor(string monitorId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            return GetMarketplaceResourceIdByMonitorAsync(monitorId, extension, updater, endpoint).Result;
        }

        private async Task<IAcisSMEOperationResponse> GetMarketplaceResourceIdByMonitorAsync(string monitorId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            if (extension == null)
            {
            ...
            var result = await coordinator.StartWorkAsync(nameof(GetMarketplaceResourceIdByMonitor), parameters: monitorId);
            if (result.Succeeded)
            {
