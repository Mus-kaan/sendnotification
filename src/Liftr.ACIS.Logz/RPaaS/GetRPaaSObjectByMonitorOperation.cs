...
namespace Microsoft.Liftr.ACIS.Logz
{
    ...
    /// </summary>
    public class GetRPaaSObjectByMonitorOperation : AcisSMEOperation
    {
        public override string OperationName => "GetRPaaSObjectByMonitor";
        ...
        /// </summary>
        public IAcisSMEOperationResponse GetRPaaSObjectByMonitor(string monitorId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            return GetRPaaSObjectByMonitorAsync(monitorId, extension, updater, endpoint).Result;
        }

        private async Task<IAcisSMEOperationResponse> GetRPaaSObjectByMonitorAsync(string monitorId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            if (extension == null)
            {
            ...
            var result = await coordinator.StartWorkAsync(nameof(GetRPaaSObjectByMonitor), parameters: monitorId);
            if (result.Succeeded)
            {
