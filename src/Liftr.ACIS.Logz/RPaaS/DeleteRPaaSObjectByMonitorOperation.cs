...
namespace Microsoft.Liftr.ACIS.Logz
{
    ...
    /// </summary>
    public class DeleteRPaaSObjectByMonitorOperation : AcisSMEOperation
    {
        public override string OperationName => "DeleteRPaaSObjectByMonitor";
        ...
        /// </summary>
        public IAcisSMEOperationResponse DeleteRPaaSObjectByMonitor(string monitorId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            return DeleteRPaaSObjectByMonitorAsync(monitorId, extension, updater, endpoint).Result;
        }

        private async Task<IAcisSMEOperationResponse> DeleteRPaaSObjectByMonitorAsync(string monitorId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            if (extension == null)
            {
            ...
            var result = await coordinator.StartWorkAsync(nameof(DeleteRPaaSObjectByMonitor), parameters: monitorId);
            if (result.Succeeded)
            {
