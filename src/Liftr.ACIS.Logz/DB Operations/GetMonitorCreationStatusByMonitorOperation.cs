...
namespace Microsoft.Liftr.ACIS.Logz
{
    ...
    /// </summary>
    public class GetMonitorCreationStatusByMonitorOperation : AcisSMEOperation
    {
        public override string OperationName => "GetMonitorCreationStatusByMonitor";
        ...
        /// </summary>
        public IAcisSMEOperationResponse GetMonitorCreationStatusByMonitor(string monitorId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            return GetMonitorCreationStatusByMonitorAsync(monitorId, extension, updater, endpoint).Result;
        }

        private async Task<IAcisSMEOperationResponse> GetMonitorCreationStatusByMonitorAsync(string monitorId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            if (extension == null)
            {
            ...
            var result = await coordinator.StartWorkAsync(nameof(GetMonitorCreationStatusByMonitor), parameters: monitorId);
            if (result.Succeeded)
            {
