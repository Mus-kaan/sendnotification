...
namespace Microsoft.Liftr.ACIS.Logz
{
    ...
    {
        public override string OperationName => "UpdateMarketplaceEntities";
        ...
        public override IEnumerable<IAcisSMEParameterRef> Parameters => new[]
            {
                ParamRefFromParam.Get<ExistingMarketplaceSaaSIdTextParameter>(isOptional: false),
                ParamRefFromParam.Get<NewMarketplaceSaaSIdTextParameter>(isOptional: false),
        ...
        /// </summary>
        public IAcisSMEOperationResponse UpdateMarketplaceEntities(string oldMarketplaceSaaSId, string marketplaceSaasId, string logzResourceId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            return UpdateMarketplaceEntitiesAsync(oldMarketplaceSaaSId, marketplaceSaasId, logzResourceId, extension, updater, endpoint).Result;
        }

        private async Task<IAcisSMEOperationResponse> UpdateMarketplaceEntitiesAsync(string oldMarketplaceSaaSId, string marketplaceSaasId, string logzResourceId, IAcisServiceManagementExtension extension = null, IAcisSMEOperationProgressUpdater updater = null, IAcisSMEEndpoint endpoint = null)
        {
            if (extension == null)
            {
            ...
            var result = await coordinator.StartWorkAsync(nameof(UpdateMarketplaceEntities), parameters: ConcatParams(oldMarketplaceSaaSId, marketplaceSaasId, logzResourceId));
            if (result.Succeeded)
            {
